...
__device__ __host__ __forceinline__ void do_move_ab(Board *board, Move *move, Undo *undo) {
...
__device__ __host__ __forceinline__ void undo_move_ab(Board *board, Move *move, Undo *undo) {
    //TOGGLE_HASH(board);
    const int coeff[2] = {1, -1};
...
__device__ __host__ __forceinline__ int is_illegal_ab(Board *board){
     return is_check_ab(board, board->color ^ BLACK);
}
...
__device__ __forceinline__ void alpha_beta_gpu_iter(Board *board_parent, Move *first_moves, Move *second_moves, int *pos_parent, int *scores_parent, int depth, int alpha_parent, int beta_parent, int nmoves, int baseIdx)
{
    ...
    {
        count = gen_moves_ab(&board, &(moves[0]));
        curr_idx = count - 1;
    }
    ...
    while (curr_idx >= 0)
    {
        ...
        while (count != 0 && !moves[curr_idx].already_executed)
        {
            ...
            board_illegal = is_illegal_ab(&board);
            if (curr_depth > 0 && !board_illegal)
            {
                count = gen_moves_ab(&board, &(moves[curr_idx + 1]));
                curr_idx += count;
            }
        }
        // terminal nodes
        ...
        if (moves[curr_idx].already_executed || curr_idx < 0)
        {
            if (!beta_reached[curr_depth])
            {
                ...
                {
                    if (is_check_ab(&board, board.color))
                    {
